class Producto {
    ...
    set precio(valor) {
        ...
        if (nuevoPrecio > 0) {
            ...
            console.error(`El precio debe ser mayor a 0. Se mantuvo el valor anterior: $${this.#precio.toFixed(2)}`);
        }
    }
    ...
    set stock(valor) {
        if (valor >= 0) {
            ...
            console.error(`El stock no puede ser negativo. Se mantuvo el valor anterior: ${this.#stock}`);
        }
    }

    vender(cantidad) {
        if (cantidad <= 0 || !Number.isInteger(cantidad)) {
            console.log(`ERROR: Cantidad de venta inválida.`);
            return false;
        }
